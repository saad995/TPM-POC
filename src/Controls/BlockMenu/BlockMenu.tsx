...
const BlockMenu = (props: IBlockMenuProps) => {
    ...
    const getBlockMenuUrl = (value: IUserMenu) => {
        let menuItem = dashboardMenu.menu.filter((a) => a.ID === value.ParentMenuID);
    ...
    return (
        <Row className="mt-3 block-menu">
            {props.menu?.map((value, index) => {
                return (
                    <Col className="mb-4 block-menu-item" style={{ minHeight: "100%" }} xs="12" sm="6" lg="4" xl="auto">
                        <NavLink to={getBlockMenuUrl(value)} key={index}>
                            <Row className="no-gutters">
                                <Col className="d-flex" style={{ minHeight: "100%" }}>
