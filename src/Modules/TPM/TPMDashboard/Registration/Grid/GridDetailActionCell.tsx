...
import { Agency, DocumentClassificationCode, DocumentGroupCode, DocumentInfo } from "Modules/Common/Constants/Interfaces";
import { Tabs } from "Modules/TPM/Common/TabStripCommon/TabStripInterface";
import Paths from "Modules/TPM/Constants/Paths";

const GridDetailActionCell = (props: any) => {
    const { rowData, agencyId, IStatusInterface, isDetailAction, handleDetailItemView, URL } = props;
   
    const { dataItem } = rowData;
    const { id, itemId, requestDocumentTypeCode, currentRecordEncryptedId, currentRecordId, parentDocumentTypeCode, statusId, initDocumentTypeCode, agencyBusinessRegistrationId} = dataItem;
    ...
    console.log("dataItem--",dataItem, "history--",history.location.state, 'localStorageData--',localStorageData)
    ...
    const amendmentRequesthandler = () => {
        ...
        return history.push(Paths.Registration.Amendment, data);
    };
    ...
    const renewalRequesthandler = () => {
        ...
        return history.push(Paths.Registration.Renewal, data);
...
//         return history.push(URL.ViewItem, data);
    ...
    switch (statusId) {
        ...
        //     }
        //     break;
        case IStatusInterface.SUBMITTED:
        case IStatusInterface.ACTIVE:
            {
                Actions = (
                    <td className="grid-column">
                        {dataItem.isAmendmentAllowed && (
                            <Button primary={true} look="flat" onClick={amendmentRequesthandler}>
                                <ImportSVG
                                    ...
                                    color="#6c757d"
                                    size={16}
                                    title={resources_EN.amendmend_request}
                                />
                            </Button>
                        )}
                        ...
                        <Button look="flat" onClick={handleDetailView}>
                            <FontAwesomeIcon
                                icon={faEye}
                                title={resources_EN.TPM_Grid_Registration_Grid_ColumnButtonName_View}
                            />
                        </Button>
                    </td>
                );
        ...
        default:
            {
                Actions = (
                    <td colSpan={1} role="gridcell" aria-selected="false">
                        {isDetailAction ? (
...
export default GridDetailActionCell;
