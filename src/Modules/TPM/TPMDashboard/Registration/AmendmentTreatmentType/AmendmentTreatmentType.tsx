...
// SCSS
import "./AmendmentTreatmentType.scss";
...
import { AmendmentFormConfig } from "./AmendmentFormConfig";
...
import { TreatmentPurpose, TreatmentTypeRegistrationStatus } from "./AmendmentTreatmentTypeInterfaces";
import { deleteTreatmentTypeRegistrationAction, saveTreatmentTypeLicenseGridDataAction, saveTreatmentTypeRegistrationAction, submitRenewalTreatmentTypeAction } from "./AmendmentTreatmentTypeAction";
import SearchCriteriaPanelComponent from "../CreateEditTPMRegistration/RegistrationDetails/SearchCriteriaPanelComponent";

const AmendmentTreatmentType = (props: any) => {
    ...
    const handleSaveTreatmentType = ({formData, data}:any) => {
        console.log("sellleecf",formData);
    ...
    const saveFormDataMapper = (formData:any, data:any)=>{
        ...
        const {documentTypeCode, treatmentTypeRegistrationId, treatmentTypeId, name, isSubType} = data;
        const {cellNumber, city, emailAddress,  officeAddress,  phoneNumber,  plantAddress,  plantCapacity,  productionUnitCode, validUpto, ispmMark, treatmentOperatorLicenseNumber, treatmentOperatorName, fumigationAllowed,treatmentOperatorInfo,treatmentTypeAttachments,treatmentSubTypes} = formData;
        ...
        if (isConfirm) {
            if (type === Action.Delete) {
                ...
                req = {
                    ...
                    purpose:TreatmentPurpose.AMENDMENT,
                ...
                dispatch(submitRenewalTreatmentTypeAction(req,(res:any)=>{
                    if (res?.hasOwnProperty("code") ? res.code !== ApplicationCodes.SUCCESS : res?.hasOwnProperty("message") ? res?.message?.code !== ApplicationCodes.SUCCESS : false) {
                        ...
                        swal({
                            title: resources_EN.sweet_Alert_title_Amend,
                            text:
                                message.code == ApplicationCodes.SUCCESS
                                    ? message.description
                                    : resources_EN.TPM_amendment_save_msg,
                            icon: "success",
                        }).then((confirm) => {
                            if(confirm)
                            {
    ...
    useEffect(()=>{
        if(getTreatmentTypesByTPId && Object.keys(getTreatmentTypesByTPId).length > 0){
            setFormConfig(AmendmentFormConfig);
...
export default AmendmentTreatmentType;
